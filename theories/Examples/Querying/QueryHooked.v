...
by case/andP:W_valid=>->_; rewrite andTb.
Qed.

Lemma W_dom : dom W.1 =i [:: plab pc; lq].
Proof.
move=>z/=; rewrite domUn !inE/=; case/andP: W_valid=>->/=_.
by rewrite !domPt !inE; rewrite -!(eq_sym z) orbC.
Qed.
...
- by move=>z lc ls t; rewrite domPt inE dom0.
- by move=>z lc ls t; rewrite domPt inE dom0.
...
Proof.
case/andP: W_valid=>V1 _.
by rewrite domUn !inE V1/= !domPt !inE eq_refl orbC.
Qed.

Lemma injWQ : inj_ext injW = (lq \\-> pq, Unit).
Proof.
move: (W_valid)=>V; move: (cohK injW).
rewrite {1}eqW/mkWorld/= -!joinA in V.
case/andP: V=>/=V V'.
rewrite {1}eqW/mkWorld/= -!joinA; case=>H K.
...
- by split; move=>z'; rewrite /getStatelet find_umfiltk domPt inE eqxx.
...
- rewrite /Actions.send_act_safe/=.
  ...
  move=>z lc hk; rewrite find_umfiltk eqxx /query_hookz.
  move=>F.
  apply sym_eq in F.
  move: F.
...
have P3: lq \in dom i1.
  rewrite -(cohD C1) domUn inE !domPt !inE /= eqxx orbC andbC/=.
